...
public abstract class Day<T>
{
    ...
    public async Task Part1()
    {
        ...
        // Actual
        Console.WriteLine("\n\n\nActual 1:\n");
        var userAnswer = DoPart1(ParseInput(aoc.Input));

        var answerOnPage = aoc.FindAnswer(1);
        if (answerOnPage == null)
        {
            logger.LogInformation("{Year}-{Day}-{Part}: Answer not found on website, submitting {Answer}", year, day, 1,
                userAnswer);
    ...
    public async Task Part2()
    {
        ...
        if (!skipExamples)
        {
            ...
            logger.LogInformation("{Year}-{Day}-{Part}: Example answer found on website, checking that instead: '{Answer}' == '{User}'", year, day, 2, exampleAnswer, userExampleAnswer);
            Assert.AreEqual(exampleAnswer, userExampleAnswer.ToString(), "Example Part 2 Failed");
        }
        ...
        // Actual
        Console.WriteLine("\n\n\nActual 2:\n");
        var userAnswer = DoPart2(ParseInput2(aoc.Input));
